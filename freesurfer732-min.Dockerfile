...
RUN echo "Downloading FreeSurfer ..." \
    ...
    && gdown https://drive.google.com/file/d/1Ro46WmJXw_3KnitRYXBihVylWJJpwFQb/view?usp=sharing -O /opt/freesurfer-7.3.1/freesurfer.tar.gz 
RUN ls /opt/freesurfer-7.3.1
RUN tar -xz  /opt/freesurfer-7.3.1/freesurfer.tar.gz -C /opt/freesurfer-7.3.1 --owner root --group root --no-same-owner --strip-components 1 \
         ...
         --exclude='trctrain' \
    && rm -f /opt/freesurfer-7.3.1/freesurfer.tar.gz
